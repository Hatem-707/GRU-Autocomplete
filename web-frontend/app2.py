...
class AutocompleteModel:
    ...
    def load_resources(self, model_path, vocab_path):
        try:
            # Load Tokenizer from tokens.json
            from tokenizers import Tokenizer

            self.tokenizer = Tokenizer.from_file(self.tokens_path)

            # Load Vocabulary for reverse lookup (id -> word)
    ...
    def tokenize_and_encode(self, text):
        """Tokenize text and encode to IDs using the tokenizer"""
        # Use the tokenizer's built-in encode method
        encoded = self.tokenizer.encode(text)
        ids = encoded.ids

        # Ensure all IDs are within valid range [0, vocab_size-1]
